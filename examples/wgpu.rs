...
use wgpu::{util::DeviceExt, ColorTargetState, BlendState};
use winit::{
...
async fn run(event_loop: EventLoop<()>, window: Window) {
    ...
    // println!("vs: {:?}", vs);
    // println!("fs: {:?}", fs);
    let path = CString::new("./source/msyh.ttf").unwrap();
    let ft_lib = (*FT_LIB.read().unwrap()) as *mut c_void;

    let mut ft_face = ptr::null_mut();
    let error = unsafe { FT_New_Face(ft_lib, path.as_ptr() as *const c_char, 0, &mut ft_face) };
    assert_eq!(error, 0);


    let time = std::time::Instant::now();
    let mut gi = GlyphInfo::new();
    let arcs = get_char_arc(&mut gi, ft_face, '魔', None);
    ...
    let char_size = 64.0;
    let mut world_matrix = nalgebra::Matrix4::<f32>::identity();
    world_matrix =
    ...
    let view_matrix = nalgebra::Matrix4::<f32>::identity();
    ...
    let proj_matrix = nalgebra::Orthographic3::<f32>::new(
        0.0,
    ...
    println!("proj_matrix.as_slice(): {:?}", proj_matrix.as_matrix().as_slice());
...
fn main() {
    ...
    let window = WindowBuilder::new().with_inner_size(winit::dpi::PhysicalSize::new(512, 512)).build(&event_loop).unwrap();
