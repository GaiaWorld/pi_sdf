...
void main() {
    ...
    // #endif
    vec4 outer_glow_color = vec4 (1.0, 0.84, 0.0, 1.0);
    vec4 fill_color = vec4 (0.0, 1.0, 0.0, 1.0);

    // fragColor =  mix(outer_glow_color, fill_color,  clamp(sdf, 0.0, 1.0));
	fragColor = vec4(1.0, 0.0, 0.0, clamp(sdf, 0.0, 1.0));
