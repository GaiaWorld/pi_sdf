...
use usvg::{
    ...
    Fill, NodeKind, Stroke, TreeParsing,
};
...
impl Svg2 {
                    ...
                    if !is_close {
                        for s in path.data.segments() {
                            match s {
                                ...
                                tiny_skia_path::PathSegment::QuadTo(c, to) => sink.quadratic_curve_to(
    ...
    pub fn compute_endpoints(&mut self) -> Vec<GlyphVisitor> {
        let mut sinks = vec![];
...
fn compute_outline<'a>(
    mut points: impl Iterator<Item = &'a tiny_skia_path::Point>,
    verbs: impl Iterator<Item = &'a tiny_skia_path::PathVerb>,
